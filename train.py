'''
Train a universal transformer-based model for emotion classification (Twitter)
Universal Transformer hre means the weights for all layers are identical.
'''
...
from models import ElectraUTClassifier
...
if __name__ == "__main__":
    ...
    commandLineParser.add_argument('--layers', type=int, default=12, help='Specify number of layers for universal transformer')
    ...
    if arch == 'electra':
        model = ElectraUTClassifier(device, layers=layers)
    else:
        raise Exception("Only electra supported so far.")
    model.to(device)
